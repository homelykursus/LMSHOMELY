...
export class ImageProcessor {
   ...
   * @param inputPath - Path to original student photo
   ...
   * @returns string | null - Full path to photo or null if not available
   */
  ...
  static async prepareStudentPhotoForCertificate(
    ...
    try {
      ...
      const originalPhotoPath = this.getStudentPhotoPath(student.photo);
      ...
      const isValid = await this.isValidImage(originalPhotoPath);
      if (!isValid) {
        console.log(`Invalid photo for student ${student.studentNumber}: ${originalPhotoPath}`);
        return null;
      }
      ...
      await this.processStudentPhoto(originalPhotoPath, processedPhotoPath, {
        width: 150,
        height: 200,
        quality: 85,
        format: 'jpeg',
