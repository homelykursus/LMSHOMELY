...
export default function ClassesManagement() {
  ...
  const getAttendanceDisabledState = (classData: Class) => {
    ...
    if (classData.teacher.status !== 'active') {
      return {
        disabled: true,
        title: 'Guru sedang tidak aktif - silakan ganti guru atau aktifkan kembali'
      };
    }
  ...
  const handleAttendanceClick = (classData: Class) => {
    ...
    if (!classData.teacher) {
      toast.error('Guru belum ditambahkan! Silakan tambahkan guru terlebih dahulu sebelum melakukan absensi.');
      return;
    }
    
    // Validasi apakah guru masih aktif
    if (classData.teacher.status !== 'active') {
      toast.error('Guru sedang tidak aktif! Silakan ganti dengan guru lain atau aktifkan kembali guru ini sebelum melakukan absensi.');
      return;
    }
                              ...
                              {classData.teacher ? (
                                <>
                                  <div className="flex items-center gap-2">
                                    <div className="font-medium">{classData.teacher.name}</div>
                                    <Badge 
                                      variant={classData.teacher.status === 'active' ? 'default' : 'destructive'}
                                      className={classData.teacher.status === 'active' 
                                        ? 'bg-green-100 text-green-800 border-green-200' 
                                        : 'bg-red-100 text-red-800 border-red-200'
                                      }
                                    >
                                      {classData.teacher.status === 'active' ? 'Aktif' : 'Tidak Aktif'}
                                    </Badge>
                                  </div>
                                  <div className="text-sm text-gray-500">{classData.teacher.education}</div>
                                  {classData.teacher.specialization && (
                                    <div className="text-xs text-blue-600">{classData.teacher.specialization}</div>
                                  )}
                                </>
                              ) : (
                                <div className="text-sm text-gray-500 italic">Belum ada guru</div>
                              )}
                            </div>
                          </TableCell>
                          <TableCell>
                    ...
                    <div>
                      ...
                      <div className="mt-2">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-sm text-green-600 font-semibold">100% Selesai</span>
                        </div>
