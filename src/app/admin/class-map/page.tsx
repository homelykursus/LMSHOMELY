...
export default function ClassMap() {
  ...
  const teacherAndRoomFilteredClasses = classes.filter(c => {
    const matchTeacher = selectedTeacher === 'all' || (c.teacher?.id === selectedTeacher) || (selectedTeacher === 'none' && !c.teacher);
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-4 items-end">
            ...
            {(selectedTeacher !== 'all' || selectedRoom !== 'all' || selectedStatus !== 'all') && (
              <Button 
                variant="ghost" 
                onClick={() => {
