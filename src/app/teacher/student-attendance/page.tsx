...
export default function TeacherStudentAttendancePage() {
  ...
  const fetchStudents = async () => {
    try {
      const response = await fetch('/api/teacher/student-attendance?type=students')
      if (response.ok) {
  ...
  const fetchAttendanceRecords = async () => {
    try {
      const response = await fetch('/api/teacher/student-attendance?type=records')
      if (response.ok) {
        const data = await response.json()
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('/api/teacher/student-attendance?type=stats')
      if (response.ok) {
        const data = await response.json()
        setStats(data)
      }
  ...
  const fetchClasses = async () => {
    try {
      const response = await fetch('/api/teacher/student-attendance?type=classes')
      if (response.ok) {
        const data = await response.json()
        setClasses(data || [])
      }
  ...
  const fetchCourses = async () => {
    try {
      const response = await fetch('/api/teacher/student-attendance?type=courses')
      if (response.ok) {
        const data = await response.json()
        setCourses(data || [])
      }
  ...
  const handleUpdateRecord = async () => {
    ...
    setIsUpdating(true)
    try {
      const response = await fetch(`/api/teacher/student-attendance/${editingRecord.id}`, {
        method: 'PUT',
        headers: {
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600">Kelola dan pantau kehadiran siswa di kelas Anda</p>
            </div>
            <Button
              onClick={refreshData}
              variant="outline"
                      ...
                      {studentsPaginatedData.map((student, index) => {
                        ...
                        return (
                          <TableRow key={student.id}>
                            ...
                            <TableCell className="font-normal">{student.studentNumber}</TableCell>
              ...
              <CardHeader>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    ...
                    <CardDescription>
                      Menampilkan {filteredAttendance.length} record absensi di kelas Anda
                    </CardDescription>
                  </div>
